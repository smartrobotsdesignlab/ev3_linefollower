...
# ---------------------------------------------------------------------------
#  PID AND FEEDFORWARD GAINS (tune these only) -- Below are the best parameters
# ---------------------------------------------------------------------------
KF_DRIVE_SPEED   = -0.01   # Feed-forward (bias) term
KD_BODY_RATE     =  0.8    # Derivative term on body angular rate
KP_BODY_ANGLE    =  15.0   # Proportional term on body angle
KD_WHEEL_RATE    =  0.08   # Derivative term on wheel rate
KP_WHEEL_ANGLE   =  0.12   # Integral-like term on wheel angle
# ---------------------------------------------------------------------------
...
while True:
    ...
    while True:
        ...
        #output_power = (-0.01 * drive_speed) + (0.8 * robot_body_rate +
                                                ...
                                                #0.08 * wheel_rate +
                                                #0.12 * wheel_angle)
        output_power = (KF_DRIVE_SPEED * drive_speed
                        + KD_BODY_RATE   * robot_body_rate
                        + KP_BODY_ANGLE  * robot_body_angle
                        + KD_WHEEL_RATE  * wheel_rate
                        + KP_WHEEL_ANGLE * wheel_angle)
